...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$("#uptBtn").click(function(){
			if(confirm("수정하시겠습니까")){
				$("form").attr("action","updateProdt_mng");
				$("form").submit();
			}
		})
		$("#delBtn").click(function(){
			if(confirm("삭제하시겠습니까")){
				$("form").attr("action","deleteProdt_mng");
				$("form").submit();
			}
		})		
	});
...
<div class="container">
	...
	<div class="input-group mb-3">	
		...
		<input name="pmng_no" readonly class="form-control"  value="${prodt_mng.pmng_no}" readonly/>	
	</div>	
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">제품코드</span>
		</div>
		<input name="no" class="form-control"   value="${prodt_mng.no}"   />	
	</div>		
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">입출고절대수량</span>
		</div>
		<input name="inc_rels_cnt" type="number" class="form-control"   value="${prodt_mng.inc_rels_cnt}"   />	
	</div>		
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">입출고구분(입고1/출고2)</span>
		</div>
		<input name="inc_rels_sort" type="number" class="form-control"   value="${prodt_mng.inc_rels_sort}" />	
	</div>			
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">현재재고량</span>
		</div>
		<input name="curr_tot_stock" type="number" class="form-control"   value="${prodt_mng.curr_tot_stock}" />	
	</div>			
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
		...
		<input type="date" name="inc_rels_dteS" class="form-control" 
			...
			value='${prodt_mng.inc_rels_dte}' pattern='yyyy-MM-dd'/>" />	
	</div>
	...
	<div class="input-group mb-3">	
		...
		<input name="prd_id_prc" type="number" class="form-control"   value="${prodt_mng.prd_id_prc}" />	
	</div>			
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">담당자번호</span>
		</div>
		<input name="empno" type="number" class="form-control"   value="${prodt_mng.empno}" />	
	</div>			
