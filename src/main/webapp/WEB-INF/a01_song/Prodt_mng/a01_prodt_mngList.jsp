...
<div class="container">
	  <h2 class="text-center">제품입출고기록 리스트</h2>
	...
	<input type="hidden" name="curPage" value="${sch.curPage}" /> 
  	...
  	<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
	    ...
	    <c:if test="${sessionScope.auth == 2 && sessionScope.deptno == 30}">
 			<button class="btn btn-success" onclick="location.href='/insertProdt_mng'" type="button">등록</button>
      	</c:if>
 	</nav>
	</form>
	<div class="container-fluid">
   ...
   <table class="table table-hover table-striped">
    ...
    <tbody>
    	<c:forEach var="prodt_mng" items="${prodt_mngList}">
    	<tr ondblclick="goDetail('${prodt_mng.pmng_no}')"><td>${prodt_mng.pmng_no}</td><td>${prodt_mng.no}</td>
    	<td>${prodt_mng.inc_rels_cnt}</td>
    	<td>${prodt_mng.inc_rels_sort}</td><td class="stock">${prodt_mng.curr_tot_stock}</td>
    	
    	<td><fmt:formatDate value='${prodt_mng.inc_rels_dte}' pattern='yyyy-MM-dd'/></td>
    	<td>${prodt_mng.prd_id_prc}</td><td>${prodt_mng.empno}</td></tr>
    	</c:forEach>
    </tbody>
	</table>
	<div class="container-fluid">
...
<script type="text/javascript">
	function goDetail(pmng_no){
		location.href="prodt_mngDetail?pmng_no="+pmng_no
	}
