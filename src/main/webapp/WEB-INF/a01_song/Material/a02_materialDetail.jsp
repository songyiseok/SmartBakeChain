...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$("#uptBtn").click(function(){
			if(confirm("수정하시겠습니까")){
				$("form").attr("action","updateMaterial");
				$("form").submit();
			}
		})
		$("#delBtn").click(function(){
			if(confirm("삭제하시겠습니까")){
				$("form").attr("action","deleteMaterial");
				$("form").submit();
			}
		})		
	});
...
<div class="container">
	...
	<div class="input-group mb-3">	
		...
		<input name="mnum" readonly class="form-control"  value="${material.mnum}" readonly/>	
	</div>	
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">원재료명</span>
		</div>
		<input name="mname" class="form-control"   value="${material.mname}"   />	
	</div>		
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">단위</span>
		</div>
		<input name="unit" class="form-control"   value="${material.unit}"   />	
	</div>		
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">가격</span>
		</div>
		<input name="mprice" type="number" class="form-control"   value="${material.mprice}" />	
	</div>			
		...
		<c:if test="${sessionScope.auth == 2 && sessionScope.deptno == 30}">
           <input type="button" id="uptBtn" class="btn btn-primary" value="수정" />
			...
			<input type="button" class="btn btn-info" 
				onclick="location.href='materialList'" 
